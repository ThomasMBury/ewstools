...
def test_ews_compute():
    ...
    lag_times = [1,2,3,4,5]
    dict_ews = ewstools.core.ews_compute(series,
                             ews=ews,
                             lag_times=lag_times,
                             sweep = True
                             )
...
def test_pspec_welch():
    ...
    pspec = ewstools.helpers.pspec_welch(yVals, dt, ham_length=ham_length)
...
def test_psd_forms():
    ...
    mu = -0.1
    w0 = 1
    sFoldVals = ewstools.helpers.psd_fold(wVals, sigma, lamda)
    sHopfVals = ewstools.helpers.psd_hopf(wVals, sigma, mu, w0)
    sNullVals = ewstools.helpers.psd_null(wVals, sigma)
...
def test_sfold_init():
    ...
    [sigma, lamda] = ewstools.helpers.sfold_init(smax, stot)
...
def test_shopf_init():
    ...
    [sigma, mu, w0] = ewstools.helpers.shopf_init(smax, stot, wdom)
...
def test_snull_init():
    ...
    [sigma] = ewstools.helpers.snull_init(stot)
...
def test_fit_fold():
    ...
    [aic, model] = ewstools.helpers.fit_fold(pspec, init)
...
def test_fit_hopf():
    ...
    [aic, model] = ewstools.helpers.fit_hopf(pspec, init)
...
def test_fit_null():
    ...
    [aic, model] = ewstools.helperfuns.fit_null(pspec, init)
...
def test_aic_weights():
    ...
    aic_weights = ewstools.helpers.aic_weights(aic_scores)
...
def test_pspec_metrics():
    ...
    spec_ews = ewstools.helperfuns.pspec_metrics(pspec,
                      ews=['smax','cf','aic'],
                      sweep=True)
...
def test_block_bootstrap():
    ...
    for bs_type in bs_types:
        samples = ewstools.core.block_bootstrap(series,
                                       n_samples,
                                       bs_type=bs_type,
...
def test_roll_bootstrap():
    ...
    # Run function
    df_bootstrap = ewstools.core.roll_bootstrap(series,
                                           n_samples=n_samples)
...
def test_mean_ci():
    ...
    intervals = ewstools.core.mean_ci(data)
